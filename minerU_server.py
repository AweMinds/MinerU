...
class MinerUService(ls.LitAPI):
    ...
    async def task_processor(self):
        ...
        while True:
            try:
                # 获取下一个任务
                task_id = await self.task_queue.get_task()
            ...
            # 短暂休息，避免空队列时过度循环
            await asyncio.sleep(0.1)  # Reduced sleep time for faster task processing
    ...
    async def process_pdf_task(self, pdf_bytes, mode, task_id):
        """异步处理PDF任务"""
        # Log the task assignment for debugging
        logger.info(f"Assigning task {task_id} to GPU {self.gpu_id}")
